...
double windowScale(HWND window) {
    if (!window) return 1.0;
    const double scale = static_cast<double>(GetDpiForWindow(window)) / 96.0;
    return scale > 0.0 ? scale : 1.0;
}

double logicalPointerCoordinate(HWND window, LONG coordinate) {
    return static_cast<double>(coordinate) / windowScale(window);
}
...
double NativeGameSurface::scale() const { DXGI_SWAP_CHAIN_DESC d{}; return SUCCEEDED(impl_->swapChain->GetDesc(&d)) ? windowScale(d.OutputWindow) : 1.0; }
...
struct AppRuntime { HWND window=nullptr; std::shared_ptr<NativeGameSurface> surface; std::shared_ptr<NativeInputState> input; doof::callback<void(std::shared_ptr<NativeGameEvent>,std::shared_ptr<NativeInputState>)> onEvent; doof::callback<void(std::shared_ptr<NativeGameSurface>,std::shared_ptr<NativeInputState>)> onRender; doof::callback<int32_t()> drain; std::atomic<bool> running{false},render{true}; bool continuous=true; bool panGestureActive=false; double panX=0,panY=0; std::atomic<double>* fps=nullptr; int frames=0; std::chrono::steady_clock::time_point fpsStart=std::chrono::steady_clock::now(); };
...
void beginGameAppPanGesture(double x,double y){if(auto r=running()){r->panGestureActive=true;r->panX=x;r->panY=y;r->input->setMousePosition(x,y);}}
void updateGameAppPanGesture(double x,double y){if(auto r=running()){if(!r->panGestureActive)return;double dx=x-r->panX,dy=y-r->panY;r->panX=x;r->panY=y;r->input->setMousePosition(x,y);if(std::abs(dx)<=0.01&&std::abs(dy)<=0.01)return;r->input->addPanDelta(dx,dy);if(r->running.load())r->onEvent.call(std::make_shared<NativeGameEvent>(kKindPan,kKeyUnknown,3,x,y,0,0,dx,dy),r->input);}}
void endGameAppPanGesture(){if(auto r=running())r->panGestureActive=false;}
void cancelGameAppPanGesture(){if(auto r=running())r->panGestureActive=false;}
