...
struct NativeGameApp::Impl{std::string error;std::shared_ptr<NativeGameSurface> surface;std::shared_ptr<NativeInputState> input=std::make_shared<NativeInputState>();std::shared_ptr<Runtime> runtime=std::make_shared<Runtime>();};std::shared_ptr<NativeGameApp> NativeGameApp::create(const std::string& title,bool windowed,int32_t width,int32_t height){return std::shared_ptr<NativeGameApp>(new NativeGameApp(title,windowed,width,height));}NativeGameApp::NativeGameApp(const std::string& title,bool windowed,int32_t width,int32_t height):impl_(std::make_shared<Impl>()){int32_t window=doof_game_window_create(title.c_str(),std::max(1,width),std::max(1,height),windowed?1:0);if(!window){impl_->error="Failed to create browser game window";return;}impl_->surface=std::make_shared<NativeGameSurface>(reinterpret_cast<void*>(static_cast<std::uintptr_t>(window)),nullptr,nullptr);impl_->runtime->surface=impl_->surface;impl_->runtime->input=impl_->input;gRuntime=impl_->runtime;}NativeGameApp::~NativeGameApp()=default;std::shared_ptr<NativeGameSurface> NativeGameApp::surface()const{return impl_->surface;}std::shared_ptr<NativeInputState> NativeGameApp::input()const{return impl_->input;}double NativeGameApp::fps()const{return impl_->runtime->fps;}
